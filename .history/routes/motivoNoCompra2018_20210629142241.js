...
const Multiple = require('../models/Datos2018');
...
const obtenerMotivoNoCompra2018= async() => {
    try {
        const motivoNoCompraPrefieroVer2018 = await Multiple.aggregate([
            {$match: {P10_A_Total_Prefiero_Ver_Lo_Que_Compro : 0}},
            {$group: {_id: null, count:{$sum:1}}}
        ])
        const motivoNoCompraMiedoEntregarInfPer2018 = await Multiple.aggregate([
            {$match: {P10_A_Total_Miedo_Entregar_Informacion_Personal : 0}},
            {$group: {_id: null, count:{$sum:1}}}
        ])
        const motivoNoCompraDesconfianzaFormaDePago2018 = await Multiple.aggregate([
            {$match: {P10_A_Total_Desconfianza_Forma_De_Pago : 0}},
            {$group: {_id: null, count:{$sum:1}}}
        ])
        const motivoNoCompraMiedoSerEstafadoEnganiado2018 = await Multiple.aggregate([
